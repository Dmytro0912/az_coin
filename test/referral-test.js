...
describe("Referral test", function () {
  ...
  it("Should go through betting workflow with 2 users", async function () {
    ...
    expect(await usdt.balanceOf(ref.address)).to.equal(
      frontAmount.mul(BigNumber.from(1000000000)).div(totalBetsAmount).mul(totalRewards).div(BigNumber.from(1000000000))
    );
    // try to reclaim
    await lp.connect(ref).claimReward();
