...
describe("Math test", function () {
  ...
  it("Should calculate rates", async function () {
    ...
    var a = await math.getOddsFromBanks(1500000000, 3000000000, 100000, OUTCOMEWIN_INDEX, 50000000, 1e9);
    dbg(
    ...
    a = await math.getOddsFromBanks(50000000, 50000000, 100000, OUTCOMEWIN_INDEX, 50000000, 1e9);
    dbg(
    ...
    a = await math.getOddsFromBanks(50000000, 50000000, 25000000, OUTCOMEWIN_INDEX, 50000000, 1e9);
    dbg(
      "1 for 1 Bet outcome1 = %s with 5% newOdds = %s (bank1=%s bank2=%s)",
      25000000,
      utils.formatUnits(a, 9),
